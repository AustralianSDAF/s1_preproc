"""
Collection of utility functions for processing.py
...
def write_rgb_image(product: object, filename: Path):
    """
    ...
    if mst and slv:
        _write_rgb_image(product, mst, slv, filename)


def _write_rgb_image(product: object, mst: str, slv: str, filename: Path):
    """
...
def snaphu_export_gpt(
    ...
    return subprocess.run(
        [
            ...
            f"-ProwOverlap={rowoverlap}",
            f"-PcolOverlap={coloverlap}",
...
def get_subswath_burst(filepath_1: Path, filepath_2: Path, aoi: list) -> dict:
    ...
    Args:
        ...
        aoi (list): Area of Interest specified as [minlon, minlat, maxlon, maxlat]
    Returns:
        subswaths_bursts_dict (dict): Dictionary containing the first/last bursts for each subswath for each product
    """
    ...
    # Subswath and Burst Selection for filepath_2
    s1.load_zip(zip_path=str(filepath_2))
    ...
    subswaths_bursts_dict = {i: {} for i in subswaths}
    for subswath in subswaths:
        for subswath_bursts in subswath_bursts_1:
            if subswath_bursts[0] == subswath:
                subswaths_bursts_dict[subswath]["product_1"] = (
                    subswath_bursts[1],
                    subswath_bursts[2],
                )
        for subswath_bursts in subswath_bursts_2:
            if subswath_bursts[0] == subswath:
                subswaths_bursts_dict[subswath]["product_2"] = (
                    subswath_bursts[1],
                    subswath_bursts[2],
                )
